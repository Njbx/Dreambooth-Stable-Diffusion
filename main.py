...
from pytorch_lightninimport argparse, os, sys, datetime, glob, importlib, csv
import numpy as np
import time
import torch
...
if __name__ == "__main__":
    ...
    try:
        ...
        if opt.train:
            try:   
                with autocast('cuda'): trainer.fit(model, data)
            except Exception:
                melk()
                raise
        ...
        if trainer.global_rank == 0:
            print(trainer.profiler.summary())g.utilities.distributed import rank_zero_only
from pytorch_lightning.utilities import rank_zero_info
